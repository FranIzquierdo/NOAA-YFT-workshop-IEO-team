...
nc<-ldat %>% filter(Convergence<0.1)
...
ggdat<-ggdat %>% filter(Convergence<0.1) # select convergent runs
...
ggplot(data=ldat, aes(y=Likelihood, x=run, col=factor(run)))+
...
ggplot(data=ldat, aes(y=Convergence, x=run, col=factor(run)))+
  geom_hline(yintercept = 0.0001, linetype="dashed", col="grey", size=1)+
  geom_point(size=2) + theme_light()+ 
  ylab("Convergence") + xlab("run") +
...
retroModels <- SSgetoutput(dirvec=file.path(main_dir, mydata[1:10]))
