...
load(file="./data/main_data.RData")
load(file="./data/poly_grid.RData")

## world map 
world_map_data <- ne_countries(scale = "medium", returnclass = "sf")

## create dir
out_03<-paste(getwd(),"/output/besag st model", sep="")
dir.create(out_03)

# poly2nb ----------------------------------------------------------------------

## for besag model we need to calculate neighbors relationship 
grid.nb <- poly2nb(poly_grid$geometry) # just a single point (i.e., queen adjacency) by contiguity
xy<-unique(cbind(data$Centrelon, data$Centrelat))

# if(ix == 1) {
# png(paste0(out_03,"/03 besag st model queen adjacency nb_", ix,".png"), width = 1000, height = 800, res = 150)
...
## plot
if(ix == 1) {
png(paste0(out_03,"/03 besag st model graph mat_", ix,".png"), width = 1000, height = 800, res = 150)
image(inla.graph2matrix(grid_graph),xlab="",ylab="")
dev.off()

png(paste0(out_03,"/03 besag st model graph fig_", ix,".png"), width = 1000, height = 800, res = 150)
plot(grid_graph)
dev.off()
}
# model ------------------------------------------------------------------------
...
pdf(paste0(out_03,"/03 besag st model 1A_psy output.pdf"), 
    width = 8, height = 7, 
    bg = "white",         
    colormodel = "cmyk",    
    paper = "A4")          
...
## save
ggsave(paste0(out_03,"/03 besag st model 1A_psy mean besag effect.png"), dpi=300, width=6, height=6)
...
## save
ggsave(paste0(out_03,"/03 besag st model 1A_psy sd besag effect.png"), dpi=300, width=6, height=6)
...
## save
ggsave(paste0(out_03,"/03 besag st model 1A_psy psyear RW2 effect.png"), dpi=300, width=6, height=6)
...
## save
ggsave(paste0(out_03,"/03 besag st model 1A_y fitted CPUE mean.png"), dpi=300, width=6, height=6)
...
## save
ggsave(paste0(out_03,"/03 besag st model 1A_y fitted CPUE sd.png"), dpi=300, width=6, height=6)
...
df_reg<-as.data.frame(unique(cbind(data$polyID,data$regionID)))
colnames(df_reg)<-c("polyID","regionID")
data_pred<-left_join(data_pred, df_reg, by="polyID")

save(data_pred, file = file.path(out_12, paste0('datapred_SS_', ix, '.RData')))
...
## 1 area ----------------------------------------------------------------------
data_idx_1<-ddply(data_pred, .(pseudoyear, yearseas, year, seas), summarize,  
                  "sumX50"=sum(X50.), "sumSd"=sqrt(sum(sd^2)))
data_idx_1$pseudoyear<-as.numeric(data_idx_1$pseudoyear)

if(ix == 1) {

ggplot(data = data_idx_1, aes(x = pseudoyear, y = sumX50, group=1))+
   geom_line(aes(x=pseudoyear, y=sumX50), size=1)+
...
## save
ggsave(paste0(out_03,"/03 besag st model predicted CPUE index_1 area.png"), dpi=300, 
       width=6, height=6)
}
...
data_idx_4<-ddply(data_pred, .(pseudoyear, yearseas, year, seas, regionID), summarize,
                  "sumX50"=sum(X50.), "sumSd"=sqrt(sum(sd^2)))
data_idx_4$pseudoyear<-as.numeric(data_idx_4$pseudoyear)

if(ix == 1) {

ggplot(data = data_idx_4, aes(x = pseudoyear, y = sumX50, group=1))+
   geom_line(aes(x=pseudoyear, y=sumX50), size=1)+
   #geom_ribbon(aes(x = pseudoyear, ymin = meanX5, ymax = meanX95), 
   #            alpha = 0.3,fill="grey")+
   ylab("Predicted median CPUE")+
...
## save
ggsave(paste0(out_03,"/03 besag st model predicted CPUE index_ 4 areas.png"), dpi=300, 
       width=6, height=6)
}
...
## save
ggsave(paste0(out_03, "/03 besag st model predicted CPUE median (years collapsed).png")
...
## create dir
# out_032<-paste(getwd(),"/output/besag st model/predicted CPUE st", sep="")
# dir.create(out_032)
...
#    ggsave(paste0(out_032,"/predicted CPUE psyear",j,".png"))
...
# imgs <- list.files(out_032, full.names = TRUE)
...
# save.image(file = paste0(out_03,"/03 besag st model.RData"))
#load("./output/besag st model/03 besag st model.RData")
